...
class SurgeStore implements Store
{
    ...
    public function increment($key, $value = 1)
    {
        ...
        return tap((int) (unserialize($record['value']) + $value), function ($value) use ($key, $record) {
            $this->put($key, $value, $record['expiration'] - Tempora::now()->getTimestamp());
        });
    }
        ...
        foreach ($this->intervals as $key) {
            ...
            try {
                $this->forever('interval-'.$key, serialize(array_merge(
                    $interval, ['lastRefreshedAt' => Tempora::now()->getTimestamp()],
                )));
    ...
    protected function intervalShouldBeRefreshed(array $interval)
    {
        return is_null($interval['lastRefreshedAt']) ||
               (Tempora::now()->getTimestamp() - $interval['lastRefreshedAt']) >= $interval['refreshInterval'];
    }
    ...
    {
        return $record === false || $record['expiration'] <= Tempora::now()->getTimestamp();
    }
