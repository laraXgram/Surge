...
use LaraGram\Surge\RoadRunner\ServerProcessInspector;
use LaraGram\Surge\RoadRunner\ServerStateFile;
...
#[AsCommand(name: 'surge:roadrunner')]
class StartRoadRunnerCommand extends Command implements SignalableCommandInterface
{
     ...
     * @var string
     */
    public $signature = 'surge:roadrunner
     ...
     * @var string
     */
    public $description = 'Start the Surge RoadRunner server';
    ...
    public function handle(ServerProcessInspector $inspector, ServerStateFile $serverStateFile)
    {
        if (! $this->isRoadRunnerInstalled()) {
            $this->components->error('RoadRunner not installed. Please execute the `surge:install` Artisan command.');
        ...
        $server = tap(new Process(array_filter([
            ...
            '-o', 'http.middleware='.config('surge.roadrunner.http_middleware', 'static'),
    ...
    {
        $command = config('surge.roadrunner.command', 'vendor/bin/roadrunner-worker');
    ...
    {
        return config('surge.max_execution_time', '30').'s';
    }
