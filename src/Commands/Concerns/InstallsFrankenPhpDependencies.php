...
use LaraGram\Surge\FrankenPhp\Concerns\FindsFrankenPhpBinary;
...
trait InstallsFrankenPhpDependencies
{
    ...
    protected function ensureFrankenPhpBinaryIsInstalled()
    {
        ...
        if (confirm('Unable to locate FrankenPHP binary. Should Surge download the binary for your operating system?', true)) {
    ...
    protected function downloadFrankenPhpBinary()
    {
        ...
        $githubProxyPrefix = env('SURGE_GITHUB_PROXY_PREFIX', '');
    ...
    /**
     * Ensure the installed FrankenPHP binary meets Surge's requirements.
     *
    ...
    protected function ensureFrankenPhpBinaryMeetsRequirements($frankenPhpBinary)
    {
        ...
        if ($lineWithVersion === null) {
            return $this->components->warn(
                'Unable to determine the current FrankenPHP binary version. Please report this issue: https://github.com/laraxgram/surge/issues/new.',
            );
        }
        ...
        $this->components->warn("Your FrankenPHP binary version (<fg=red>$version</>) may be incompatible with Surge.");

        if (confirm('Should Surge download the latest FrankenPHP binary version for your operating system?', true)) {
