...
use LaraGram\Surge\FrankenPhp\ServerProcessInspector;
use LaraGram\Surge\FrankenPhp\ServerStateFile;
use LaraGram\Console\Attribute\AsCommand;
use LaraGram\Console\Command\SignalableCommandInterface;
use LaraGram\Console\Process\Process;

#[AsCommand(name: 'surge:frankenphp')]
class StartFrankenPhpCommand extends Command implements SignalableCommandInterface
{
     ...
     * @var string
     */
    public $signature = 'surge:frankenphp
     ...
     * @var string
     */
    public $description = 'Start the Surge FrankenPHP server';
    ...
    {
        if (! $mercure = (config('surge')['mercure'] ?? false)) {
            return '';
        }
    ...
    {
        return config('surge.max_execution_time', 30);
    }
