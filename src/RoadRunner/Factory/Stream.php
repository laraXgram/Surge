...
class Stream implements StreamInterface, Stringable
{
     ...
     * @throws Exceptions\InvalidArgumentException For stream identifier that cannot be cast to a resource.
     * @throws Exceptions\InvalidArgumentException For non-resource stream.
     */
    ...
    {
        if (! $this->resource) {
            throw Exceptions\UntellableStreamException::dueToMissingResource();
        }

        $result = ftell($this->resource);
        if (! is_int($result)) {
            throw Exceptions\UntellableStreamException::dueToPhpError();
        }
    ...
    {
        if (! $this->resource) {
            throw Exceptions\UnseekableStreamException::dueToMissingResource();
        }

        if (! $this->isSeekable()) {
            throw Exceptions\UnseekableStreamException::dueToConfiguration();
        }

        $result = fseek($this->resource, $offset, $whence);

        if (0 !== $result) {
            throw Exceptions\UnseekableStreamException::dueToPhpError();
        }
    }
    ...
    {
        if (! $this->resource) {
            throw Exceptions\UnwritableStreamException::dueToMissingResource();
        }

        if (! $this->isWritable()) {
            throw Exceptions\UnwritableStreamException::dueToConfiguration();
        }

        $result = fwrite($this->resource, $string);

        if (false === $result) {
            throw Exceptions\UnwritableStreamException::dueToPhpError();
        }
    ...
    {
        if (! $this->resource) {
            throw Exceptions\UnreadableStreamException::dueToMissingResource();
        }

        if (! $this->isReadable()) {
            throw Exceptions\UnreadableStreamException::dueToConfiguration();
        }

        $result = fread($this->resource, $length);

        if (false === $result) {
            throw Exceptions\UnreadableStreamException::dueToPhpError();
        }
     ...
     * @throws Exceptions\InvalidArgumentException For invalid streams or resources.
     */
